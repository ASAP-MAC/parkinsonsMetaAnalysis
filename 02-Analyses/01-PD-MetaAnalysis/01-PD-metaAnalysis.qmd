...
Input.tse <- addMDS(Input.tse[rownames(Input.tse) != "UNINTEGRATED"], assay.type = params$pMD_data_type, method = "bray", name = "bray")
...
Input.tse <- addPERMANOVA(
  Input.tse,
  assay.type = params$pMD_data_type,
  method = "bray",
  name = "bray_PERMANOVA",
  formula = x ~ study_name + Parkinson_1_0,
  by = "margin",
...
prevals <- getPrevalence(Input_usableDatasets.tse, assay.type = params$pMD_data_type)
  
Input_usableDatasets.tse <- Input_usableDatasets.tse[prevals >= as.numeric(params$prevalThresh),]
```
...
### write input as usable tables to try my neural network classifier

```{r}
dir.create("ML/microbiome-nn/Data", recursive = TRUE)

write_tsv(Input_usableDatasets.tse@colData %>% as.data.frame(), file = "ML/microbiome-nn/Data/colData.tsv")

readr::write_tsv(assay(Input_usableDatasets.tse, "relative_abundance") %>% t() %>% as.data.frame() %>% rownames_to_column("uuid"), file = "ML/microbiome-nn/Data/assay_relative_abundance.tsv")
```
...
per_dataset_limma_results.list <- Input_usableDatasets.tse %>% 
...
per_dataset_limma_results.list <- lapply(per_dataset_limma_results.list, function(x) dplyr::right_join(as.data.frame(rowData(Input_usableDatasets.tse)) %>% rownames_to_column("FeatureID"), x, by = "FeatureID"))
...
meta_analysis_results <-dplyr::right_join(as.data.frame(rowData(Input_usableDatasets.tse)) %>% rownames_to_column("FeatureID"), meta_analysis_results, by = "FeatureID")
...
legend_table <- Input_usableDatasets.tse %>% 
  colData %>% 
  as_tibble %>% 
  group_by(study_name) %>% 
...
ForestPlot <- biobakeryForestPlot(
  ...
  enrich_y_taxonomy_label(taxonomy.df = meta_analysis_results[,1:(which(colnames(meta_analysis_results) == "RE_Eff.Size") - 1)])

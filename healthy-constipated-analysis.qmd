...
```{r}
constipation_joined_analyses <- inner_join(wallen_fit_Controls_constipated_vs_not_constipated_5perc, slow_vs_normal_prev5pct, by = colnames(select(slow_vs_normal_prev5pct, Kingdom:SGB)), suffix = c(".Wallen", ".Asnicar"))
```
...
constipation_joined_analyses_sub <- constipation_joined_analyses %>% 
  ...
  select(Species, SGB, contains(c("CohenD", "CohenSE", "Beta", "SE", "adj.P.Value")), absDiffBetas = absDiff)
...
meta_analysis_subset_with_PD_metaAnalysis <- full_join(constipation_joined_analyses_sub, meta_analysis_subset, by = "SGB") %>% 
  relocate(PD_assoc_metaAnalysis, .after = 'SGB') %>% 

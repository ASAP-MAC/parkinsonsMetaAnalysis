...
raw.tse <- returnSamples(pMD_data_type = params$pMD_data_type,
                                            sample_data = metadata_datasets_available)

# enhance data 
parkinsonsMetaAnalysisData.tse <- pMD_enhance(input.tse = raw.tse, pMD_data_type = params$pMD_data_type, sampleMetadata.df = metadata_datasets_available)
```
...
unique_samples_union <- unique(c(
  unname(do.call(c, sapply(all_0.logical, names))),
  unname(do.call(c, sapply(anyNAs.logical, names))),
  unname(do.call(c, sapply(all_counts_UN, names)))
  ))
...
```{r, message=TRUE}
nameSuffix <- paste(parkinsonsMetagenomicData::biobakery_files()$tool[parkinsonsMetagenomicData::biobakery_files()$pMD_data_type == params$pMD_data_type], params$pMD_data_type, sep = "_")
